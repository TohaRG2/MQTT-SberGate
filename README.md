...
- Сущности light отображаются как light. Для сущностей light, если они поддерживают яркость и/или цветность, 
...
device_id. На данный момент давление в Сбер прокидывается, но его пока не умеет отображать Салют.
...
- Датчики давления в HA могут иметь не только тип "pressure", но и "atmospheric_pressure",
добавлена поддержка и таких датчиков
- Исправлена обработка input_boolean как scenario_button и button как switch
- Добавлена обработка input_button как scenario_button, что на мой взгляд гораздо логичнее, чем
делать вызов через input_boolean, правильнее вызвать input_button.press, что Салютом будет
расценено как одиночное нажатие и можно повторять это нажатие не один раз, не меняя состояние
виртуальной кнопки. Отлично работает, как через NodeRed, так и в сценариях самого Home Assistant
