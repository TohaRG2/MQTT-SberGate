...
# Глобальные переменные для вычисления принадлежности датчика комнате
# entity → device → area
HA_AREA = {}
HA_DEVICES = {}
...
def ha_on_off(id):
    on_off = DevicesDB.get_state(id, 'on_off')
    entity_domain, entity_name = id.split('.', 1)
    log('Отправляем команду в HA для ' + id + ' ON: ' + str(on_off))
...
# Подключаемся к HA через WebSocket, если успешно, то подписываемся на события
# изменения состояния (state_changed)
# ========== WebSocket ============
...
# При получении сообщения от HA, определяем тип сообщения и вызываем соответствующий обработчик
# Тип сообщения           Вызываемая функция              Назначение
# 'auth_required'         ws_auth_required                HA требует авторизацию → отправляем токен.
# 'auth_ok'               ws_auth_ok                      Авторизация прошла → подписываемся на события и запрашиваем данные (зоны, сущности).
# 'auth_invalid'          ws_auth_invalid                 Ошибка авторизации → логируем.
# 'result'                ws_result                       Ответ на запрос (например, список зон или сущностей). Обрабатываем и сохраняем.
# 'event'                 ws_event                        Произошло событие в HA (например, изменилось состояние устройства) → обновляем состояние в DevicesDB.
...
# HA требует авторизацию → отправляем токен.
def ws_auth_required(ws, mdata):
    log("WebSocket: auth_required")
    ws.send(json.dumps({"type": "auth", "access_token": Options['ha-api_token']}))

# Авторизация прошла → подписываемся на события и запрашиваем данные (зоны, устройства и сущности).
# В Home Assistant «комната» = Area (зона). Датчик (entity) → привязан к device → device привязан к area.
# Поэтому напрямую у датчика комнаты нет — её нужно получить через датчик. Получаем нужную информацию
...
# Ответ на запрос (например, список зон или сущностей). Обрабатываем и сохраняем.
...
# Произошло событие в HA (изменилось состояние устройства) → обновляем состояние
# в DevicesDB и отправляем в MQTT Sber.
def ws_event(ws, mdata):
    ...
    if id.count('sensor.temperatura_kabinet') > 0:
        log('ANY Event: ' + id + ': ' + old_state + ' -> ' + new_state)
        log('mdata' + mdata['event'])
    if not (dev is None):
        if dev['enabled']:
            log('HA Event: ' + id + ': ' + old_state + ' -> ' + new_state, 3)
...
def upd_sw(id, s):
    ...
    DevicesDB.update(s['entity_id'],
                     {'entity_ha': True, 'entity_type': 'sw', 'friendly_name': attr, 'category': 'relay'})
...
def upd_light(id, s):
    ...
    DevicesDB.update(s['entity_id'],
                     {'entity_ha': True, 'entity_type': 'light', 'friendly_name': attr, 'category': 'light'})
...
def upd_scr(id, s):
    ...
    DevicesDB.update(s['entity_id'],
                     {'entity_ha': True, 'entity_type': 'scr', 'friendly_name': attr, 'category': 'relay'})
...
def upd_sensor(id, s):
    ...
    if dc == 'temperature':
        ...
        DevicesDB.update(id, {'entity_ha': True, 'entity_type': 'sensor_temp', 'friendly_name': fn,
...
def upd_input_boolean(id, s):
    ...
    DevicesDB.update(id, {'entity_ha': True, 'entity_type': 'input_boolean', 'friendly_name': fn,
...
def upd_hvac_radiator(id, s):
    ...
    if dc == 'temperature':
        ...
        DevicesDB.update(id, {'entity_ha': True, 'entity_type': 'hvac_radiator', 'friendly_name': fn,
...
sber_types = {'FLOAT': 'float_value', 'INTEGER': 'integer_value', 'STRING': 'string_value', 'BOOL': 'bool_value',
              'ENUM': 'enum_value', 'JSON': '', 'COLOUR': 'colour_value'}
#
...
# Если локальная база устройств отсутствует, то создаем новый пустой файл
if not os.path.exists(fDevicesDB):
    json_write(fDevicesDB, {})

log('Чтение базы устройств из devices.json', 3)
DevicesDB = CDevicesDB(fDevicesDB)
AgentStatus = {"online": True, "error": "", "credentials": {'username': Options['sber-mqtt_login'], "password": "***",
...
# Для начала подключаемся к HA через REST API https://developers.home-assistant.io/docs/api/rest/
# и получаем список объектов
# url = "http://localhost:8123/ENDPOINT"
hds = {'Authorization': 'Bearer ' + Options['ha-api_token'], 'content-type': 'application/json'}
url = Options['ha-api_url'] + '/api/states'
log('Подключаемся к HA, (ha-api_url: ' + Options['ha-api_url'] + ')', 3)
cx = 0
while cx < 10:
    cx = cx + 1
    try:
        ...
        log(f'Ошибка подключения к HA. Ждём 5 сек перед повторным подключением. Попытка {cx}', 6)
        time.sleep(5)
if response.status_code == 200:
    log('Запрос устройств из Home Assistant выполнен штатно. Обрабатываем полученный список')
    ha_dev = response.json()
    log(ha_dev, 0)
else:
    log('ОШИБКА! Запрос устройств из Home Assistant выполнен некорректно.', 6)
    ha_dev = []
    log('Запрошенный URL: ' + url, 6)
    log('Код ответа сервера: ' + str(response.status_code), 6)
    # Нет смысла продолжать выполнение

for entity in ha_dev:
    entity_type, entity_name = entity['entity_id'].split('.', 1)
    dict = {
        'switch': upd_sw,
        'light': upd_light,
        'script': upd_scr,
    ...
    # Определяем тип полученной сущности из списка выше и вызываем соответствующую
    # функцию из того же списка для добавления сущности в базу
    dict.get(entity_type, upd_default)(entity['entity_id'], entity)
...
if not os.path.exists('models.json'):
    ...
    SD_Models = requests.get(Options['sber-http_api_endpoint'] + '/v1/mqtt-gate/models', headers=hds,
                             auth=(Options['sber-mqtt_login'], Options['sber-mqtt_password']))
...
def GetCategory():
    if not os.path.exists(fCategories):
        ...
        Categories = {}
        SD_Categories = requests.get(Options['sber-http_api_endpoint'] + '/v1/mqtt-gate/categories', headers=hds,
                                     auth=(Options['sber-mqtt_login'], Options['sber-mqtt_password'])).json()
        for id in SD_Categories['categories']:
            log('Получаем опции для котегории!!!: ' + id)
            SD_Features = requests.get(
                Options['sber-http_api_endpoint'] + '/v1/mqtt-gate/categories/' + id + '/features', headers=hds,
                auth=(Options['sber-mqtt_login'], Options['sber-mqtt_password'])).json()
...
# WebSocket для получения дополнительной информации от HA об устройствах и смены их состояния
